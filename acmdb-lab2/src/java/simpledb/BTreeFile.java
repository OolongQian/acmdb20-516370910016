...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// if current page is already leaf page, base case in recursion.
		if (pid.pgcateg() == BTreePageId.LEAF) {
			return (BTreeLeafPage) getPage(tid, dirtypages, pid, perm);
		} else {
			// otherwise, root is an internal.
			assert pid.pgcateg() == BTreePageId.INTERNAL;
			BTreeInternalPage internalPage = (BTreeInternalPage) page;
			
			Iterator<BTreeEntry> iter = internalPage.iterator();
			
			// assert internal node non-empty.
			assert iter.hasNext();
			
			// if f is null, return the left most.
			if (f == null) {
				return findLeafPage(tid, dirtypages, iter.next().getLeftChild(), perm, f);
			} else {
				// otherwise.
				BTreeEntry initEntry = iter.next();
				
				IndexPredicate leQuery = new IndexPredicate(Op.LESS_THAN_OR_EQ, f);
				IndexPredicate geQuery = new IndexPredicate(Op.GREATER_THAN_OR_EQ, f);
				
				if (geQuery.filter(initEntry.getKey())) {
					// if the queried is already smaller than the initial key.
					//      descend the first-left child.
					return findLeafPage(tid, dirtypages, initEntry.getLeftChild(), perm, f);
				} else {
					// otherwise, continue.
					BTreeEntry prevEntry = initEntry;
					BTreeEntry curEntry;
					
					while (iter.hasNext()) {
						curEntry = iter.next();
						
						// if queried key lies between two keys, we descend to the child in between.
						if (leQuery.filter(prevEntry.getKey()) && geQuery.filter(curEntry.getKey())) {
					...
					// queried key is greater than any key in this internal node, return the right most node.
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// see redistribute how many.
		int totalNum = page.getNumTuples() + sibling.getNumTuples();
		int shiftNum = (totalNum / 2) - page.getNumTuples();
		
		for (int i = 0; i < shiftNum; ++i) {
			// create entry to be inserted into page.
			Tuple shiftTuple;
			
			if (isRightSibling)
				shiftTuple = sibling.iterator().next();
			else
				shiftTuple = sibling.reverseIterator().next();
			
			sibling.deleteTuple(shiftTuple);
			page.insertTuple(shiftTuple);
		}
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int shiftNum = (totalNum / 2) - page.getNumEntries();
		
		for (int i = 0; i < shiftNum; ++i) {
			// create entry to be inserted into page.
			BTreeEntry shiftEntry = new BTreeEntry(parentEntry.getKey(), leftSibling.reverseIterator().next().getRightChild(), page.iterator().next().getLeftChild());
			
			// insert
			page.insertEntry(shiftEntry);
			
			// rotate and update parent entry
			parentEntry.setKey(leftSibling.reverseIterator().next().getKey());
			
			// delete left sibling's last entry
			leftSibling.deleteKeyAndRightChild(leftSibling.reverseIterator().next());
		}
		
		// really update parent entry.
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		for (int i = 0; i < shiftNum; ++i) {
			// create entry to be inserted into page.
			BTreeEntry shiftEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), rightSibling.iterator().next().getLeftChild());
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// merge entries.
		int shiftNum = rightPage.getNumTuples();
		for (int i = 0; i < shiftNum; ++i) {
			Tuple shiftTuple = rightPage.iterator().next();
			rightPage.deleteTuple(shiftTuple);
			leftPage.insertTuple(shiftTuple);
		}
		
		// maintain leaf double linked list.
		...
		// recursively delete parent entry.
		deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
		
		// mark the right page as not dirty for reuse.
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int shiftEntries = rightPage.getNumEntries();
		for (int i = 0; i < shiftEntries; ++i) {
			BTreeEntry shiftEntry = rightPage.iterator().next();
			rightPage.deleteKeyAndLeftChild(shiftEntry);
			leftPage.insertEntry(shiftEntry);
		}
