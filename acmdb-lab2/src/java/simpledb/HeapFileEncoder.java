...
public class HeapFileEncoder {
    ...
    public static void convert(ArrayList<ArrayList<Integer>> tuples, File outFile, int npagebytes, int numFields) throws IOException {
        // create a tempInput file, write tuples onto the file
        // each record takes one row, and tuple elements are separated by comma
    ...
    public static void convert(File inFile, File outFile, int npagebytes,
        ...
        // number of bytes for each record, determined by heapFile schema
        int nrecbytes = 0;
        for (int i = 0; i < numFields; i++) {
            nrecbytes += typeAr[i].getLen();
        }

        // maximum record number on one page
        ...
        // each page has header, and this is how many bytes take on the page header
        int nheaderbytes = (nrecords / 8);
        if (nheaderbytes * 8 < nrecords)
            nheaderbytes++;  //ceiling
        // similar to nheaderbytes, but is measured by bit
        ...
        boolean first = true;
        while (!done) {
            int c = br.read(); // read from input file to character c
            ...
            if (c == '\n') {
                // if '\n' in the beginning of one line
                if (first)
                    continue;
                // '\n' begins up a new line.
                recordcount++;
                first = true;
            } else {
                first = false;
            ...
            if (c == fieldSeparator || c == '\n' || c == '\r') {
                // if not separator, parse data content according to Type
                ...
                // reset curpos, reuse buffer
                curpos = 0;
                // if not first '\n', reset field for another record,
                // otherwise increment the record index
                if (c == '\n')
                    fieldNo = 0;
                else
                    fieldNo++;
                ...
                // put the content read from input temp file to buffer
                buf[curpos++] = (char) c;
                continue;
            }
            ...
            // page to disk.
            if (recordcount >= nrecords  // current page is full of record
                    || done && recordcount > 0  // the file has completed encoding
                    || done && npages == 0) {  // create at least one page for empty file
                int i = 0;
                byte headerbyte = 0;

                // write page header for record slot
