...
public class JoinOptimizer {
	...
	public Vector<LogicalJoinNode> orderJoins(
		...
		// This is a dynamic programming algorithm, in which the computeCostAndCardOfSubplan() is the core.
		PlanCache pc = new PlanCache();
		
		// initialize pc by adding mono-join plan (join two tables) as the base case.
		for (Iterator<LogicalJoinNode> iter = joins.iterator(); iter.hasNext(); ) {
			LogicalJoinNode joinNode = iter.next();
			
			Vector<LogicalJoinNode> order = new Vector<>();
			order.add(joinNode);
			
			Set<LogicalJoinNode> s = new HashSet<>(order);
			
			// use stats to query cost and card.
			TableStats stats1 = stats.get(joinNode.t1Alias);
			TableStats stats2 = stats.get(joinNode.t2Alias);
			
			int card1 = stats1.estimateTableCardinality(filterSelectivities.get(joinNode.t1Alias));
			int card2 = stats2.estimateTableCardinality(filterSelectivities.get(joinNode.t2Alias));
			
			double scanCost1 = stats1.estimateScanCost();
			double scanCost2 = stats2.estimateScanCost();
			
			double joinCost = estimateJoinCost(joinNode, card1, card2, scanCost1, scanCost2);
			
			Catalog catalog = Database.getCatalog();
			int tableId1 = catalog.getTableId(joinNode.t1Alias);
			int tableId2 = catalog.getTableId(joinNode.t2Alias);
			boolean t1pkey = joinNode.f1PureName.equalsIgnoreCase(catalog.getPrimaryKey(tableId1));
			boolean t2pkey = joinNode.f2PureName.equalsIgnoreCase(catalog.getPrimaryKey(tableId2));
			
			int joinCard = estimateJoinCardinality(joinNode, card1, card2, t1pkey, t2pkey, stats);
			
			pc.addPlan(s, joinCost, joinCard, order);
		}
		
		// if that's all, also fine, because only one joinNode in the plan (list). The order is unique.
		if (joins.size() == 1)
			return new Vector<>(joins);
		
		else {
			double bestCost = Double.MAX_VALUE;
			CostCard bestPlan = new CostCard();
			for (int size = 2; size <= joins.size(); ++size) {
				// all possible subsets.
				Set<Set<LogicalJoinNode>> subsets = enumerateSubsets(joins, size);
				Iterator<Set<LogicalJoinNode>> subsetsIter = subsets.iterator();
				while (subsetsIter.hasNext()) {
					Set<LogicalJoinNode> subset = subsetsIter.next();
					
					// try all different join order.
					Iterator<LogicalJoinNode> joinToRemoveIter = subset.iterator();
					double bestSubsetCost = Double.MAX_VALUE;
					CostCard bestSubsetPlan = new CostCard();
					
					while (joinToRemoveIter.hasNext()) {
						LogicalJoinNode joinToRemove = joinToRemoveIter.next();
						CostCard costCard = computeCostAndCardOfSubplan(stats, filterSelectivities, joinToRemove, subset, bestSubsetCost, pc);
						if (costCard == null)
							continue;
						if (costCard.cost < bestSubsetCost) {
							bestSubsetCost = costCard.cost;
							bestSubsetPlan = costCard;
						}
					}
					pc.addPlan(subset, bestSubsetPlan.cost, bestSubsetPlan.card, bestSubsetPlan.plan);
					
					if (subset.size() == joins.size() && bestSubsetCost < bestCost) {
						bestCost = bestSubsetCost;
						bestPlan = bestSubsetPlan;
					}
				}
			}
